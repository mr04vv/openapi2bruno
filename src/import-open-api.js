...
const { BrunoError } = require('./error');
const { validateSchema, transformItemsInCollection, hydrateSeqInCollection } = require( './common');
const fs = require('node:fs');


const readFile = (file) => {
  return new Promise((resolve, reject) => {
    const parsedData = jsyaml.load(file);
    resolve(parsedData);
  });
};
...
const importCollection = (filePath) => {
  return new Promise((resolve, reject) => {
